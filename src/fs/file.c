...
rollback:
    ...
    case 3: memset(&file_table[fd_idx], 0, sizeof(struct file));

    case 2: sys_free(new_file_inode);

    case 1: bitmap_set(&cur_part->inode_bitmap, inode_no, 0);
    ...
    if (flag & O_WRONLY || flag & O_RDWR) {
        ...
        if (!(*write_deny)) {
            *write_deny = true;
            ...
            intr_set_status(old_status);
            printk(
                "file_open error: file can`t be write now, try again later\n");
            return -1;
        }
    }
